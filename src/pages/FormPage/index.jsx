...
class Shipping extends React.Component {
    ...
    inputs: [
      {
        ...
        classError: '',
        validators: [
          emptyValidator, addressValidator
        ],
        error: null
      }, {
        id: '3',
  ...
  render() {
    const { products, counters } = this.props;
    const sum = products.reduce((result, product, index) => {
      ...
      <form className="form-shipping" action="" method="post">
        {
          this.state.inputs.map(definition => {
            return (<FormGroup key={definition.id} class={definition.cls} type={definition.type} title={definition.title} errorName={definition.error} classError={definition.classError} onChange={value => this.updateValue(definition, value)}/>)
          })
        }
